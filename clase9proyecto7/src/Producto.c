...
int prod_imprimir(Producto* pProducto)
{
	...
	{
		retorno = 0;
		printf("ID: %d- %s - %s - su valor es %.2f",pProducto->id, pProducto->nombre,pProducto->descripcion,pProducto->precio);
	}
	return retorno;
}
...
int prod_imprimirArray(Producto* arrayProductos,int limite)
{
	...
	if(arrayProductos != NULL && limite > 0)
	{
		...
		for(i=0;i<limite;i++)
		{
			...
			{
				printf("\nID: %d - %s - %s - su valor es %.2f",arrayProductos[i].id,arrayProductos[i].nombre,arrayProductos[i].descripcion,arrayProductos[i].precio);
